...
async registerUser(id_event, id_user, description, attended, observations, rating, registration_date_time) {
    ...
    const sqlCheckCapacity = `
        SELECT COUNT(*) AS enrolled_count, E.max_assistance
        FROM event_enrollments EE
        JOIN events E ON EE.id_event = E.id
        WHERE EE.id_event = '${id_event}'
        GROUP BY E.max_assistance;
    `;
    const capacityResponse = await this.DBClient.query(sqlCheckCapacity);
    const { enrolled_count, max_assistance } = capacityResponse.rows[0];

    if (enrolled_count >= max_assistance) {
